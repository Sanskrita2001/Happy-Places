...
import {listOffbeatPlaces} from '../actions/offbeatAction'

import OffbeatComponent from '../components/OffbeatComponent'
import Loader from '../components/Loader'
import Error from '../components/Error'

const OffbearScreen = () => {
    const dispatch = useDispatch()
    const getOffbeatPlaces = useSelector(state=>state.getOffbeatPlaces)
    const {loading, error, offbeatplaces} = getOffbeatPlaces

    useEffect(()=>{
     dispatch(listOffbeatPlaces())   
    }, [dispatch])
    return (
        <div>
				...
				{loading === false && !error && (
					<div className='absolute z-20 -top-32 p-20 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3 gap-20'>
						{offbeatplaces.map((place) => (
							<OffbeatComponent key={place.id} place={place} />
						))}
					</div>
				)}
                </section>
